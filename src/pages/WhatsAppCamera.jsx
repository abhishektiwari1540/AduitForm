...
function CameraModal({
  ...
  const initializeCamera = async () => {
    try {
      ...
      const constraints = {
        video: {
          facingMode: { ideal: cameraType === "environment" ? "environment" : "user" },
      ...
      if (isMobileDevice()) {
        setCameraError("Cannot access camera directly. Using native camera instead.");
  ...
  const switchCamera = async () => {
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        // Check if it's front camera and apply mirroring
  ...
  const captureImage = () => {
    ...
    if (isFrontCamera) {
      // For front camera: un-mirror the image
      ctx.save();
      ctx.scale(-1, 1); // Flip horizontally
      ctx.drawImage(video, -canvas.width, 0, canvas.width, canvas.height);
      ctx.restore();
    } else {
      // For rear camera: draw normally
            ...
            <div style={{
              ...
              {cameraError && (
                <div style={{
                  ...
                  <MdCamera size={48} style={{ marginBottom: '1rem' }} />
                  <h3 style={{ marginBottom: '0.5rem' }}>Camera Access Issue</h3>
